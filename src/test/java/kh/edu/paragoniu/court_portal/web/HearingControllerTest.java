...
class HearingControllerTest {
    ...
    void hearingsTabRendersUpcomingAndPastHearings() throws Exception {
        ...
        when(hearingService.findUpcomingHearings(caseId)).thenReturn(
            List.of(
                new HearingRow(
    ...
    void newHearingFormRendersOptions() throws Exception {
        UUID caseId = UUID.randomUUID();
        when(caseService.findDetail(caseId)).thenReturn(detail(caseId));
        when(hearingService.findHearingTypeOptions()).thenReturn(
            List.of(new FilterOption(1, "Arraignment"))
        );
        when(hearingService.findCourtroomOptions()).thenReturn(
            List.of(new FilterOption(1, "Courtroom A"))
        );
    ...
    void scheduleHearingRedirectsToHearingsTab() throws Exception {
        ...
        String view = new HearingController(
            caseService,
            hearingService
        ).scheduleHearing(
            caseId.toString(),
        ...
        org.assertj.core.api.Assertions.assertThat(view).isEqualTo(
            "redirect:/cases/" + caseId + "/hearings"
        );
    ...
    void rescheduleHearingFormRendersModal() throws Exception {
        ...
        when(hearingService.findRescheduleView(caseId, hearingId)).thenReturn(
    ...
    private MockMvc mockMvc() {
        return MockMvcBuilders.standaloneSetup(
            new HearingController(caseService, hearingService)
        )
                ...
                new HandlerMethodArgumentResolver() {
                    ...
                    public boolean supportsParameter(
                        ...
                        return parameter
                            .getParameterType()
                            .equals(
                                kh.edu.paragoniu.court_portal.security.GreffierUserDetails.class
                            );
                    }
